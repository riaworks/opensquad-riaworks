...
## AI-Generated Full Slides (Nano Banana)

The preferred method for creating Instagram carousel slides is **full AI generation via Nano Banana (Google Gemini)**. Each slide is generated as a complete image — text, layout, data visualizations, colors — all in one AI-generated PNG.

### When to Use Full AI Generation

- **Always** for Instagram carousels (default method)
- When slides need high visual impact with integrated text
- When filling 100% of the canvas area matters
- When consistency across slides is critical

### Prompt Engineering for Full Slides

Every prompt MUST include:

1. **Dimensions**: "Create a 3:4 portrait image (1080x1440 pixels)"
2. **Design system block**: Colors, font style, accent color, header format
3. **Background type**: Dark (#0D0D0D), Light (#F5F5F5), or Accent (#FF6600)
4. **Layout description**: Exact position of each element (headline, cards, body, source)
5. **Exact text**: Every word that should appear, with correct PT-BR accents
6. **Fill instruction**: "Fill the ENTIRE 1080x1440 canvas — no empty areas"
7. **Accent rule**: "All text in Brazilian Portuguese with correct accents (ã, õ, é, ê, í, ó, ú, ç, à, â, ô)"
8. **Style**: "Sophisticated editorial design — refined, sober, NOT aggressive infographic"

### PT-BR Accents (Blocking Rule)

All generated slides MUST have correct Brazilian Portuguese accents. This is verified post-generation:
- Check every word for correct diacritics
- If accents are wrong or missing, regenerate with more emphatic prompt
- Include in prompt: "Missing accents are a BLOCKING ERROR"

### Verification Checklist

After generating each slide:
- [ ] PT-BR accents correct on all text
- [ ] All text readable (not blurry, not cut off)
- [ ] Full 1080x1440 area utilized (no large empty spaces)
- [ ] Numbers and data rendered correctly
- [ ] Visual style consistent with other slides in the carousel
- [ ] Slide 1 content in upper portion (WhatsApp preview compatibility)

### Fallback: HTML/CSS + Playwright

If Gemini repeatedly fails to render text correctly (especially PT-BR accents), fall back to:
1. Generate backgrounds via Nano Banana (no text, mode legacy)
2. Create HTML/CSS slides with text overlay
3. Render via Playwright (image-creator skill)

This is the LEGACY method — use only when full AI generation fails.
